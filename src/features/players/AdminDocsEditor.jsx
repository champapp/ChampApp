...
import { adminDocTypesForCategory } from '../../lib/domain';
...
// de cada papel exigido según su categoría. Se guarda al instante, fila por fila.
export function AdminDocsEditor({ playerId, cat }) {
  const docsQ = useAdminDocs(playerId);
  const upsert = useUpsertAdminDoc();
  const docByType = new Map((docsQ.data ?? []).map((d) => [d.type, d]));
  const docTypes = adminDocTypesForCategory(cat);
