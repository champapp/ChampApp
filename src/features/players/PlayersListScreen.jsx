    ...
    <div style={{ padding: '4px 16px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {list.map((p) => {
          if (archived) {
            return (
              <Card key={p.id} pad={11} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <button onClick={() => reactivate(p)} style={{ display: 'flex', alignItems: 'center', gap: 5, border: `1.5px solid ${CC.line}`, background: '#fff', color: CC.navy, padding: '8px 12px', borderRadius: 10, cursor: 'pointer', fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 700, fontSize: 13.5, letterSpacing: 0.3 }}>
                  <Icon name="restore" size={15} color={CC.navy} sw={2.4} />Reactivar
                </button>
              </Card>
            );
          }
