  ...
  return (
    <Card pad={14} style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 38, height: 38, borderRadius: 10, background: rivalCrestSrc(m.rival) ? '#fff' : 'rgba(14,58,92,0.06)', border: rivalCrestSrc(m.rival) ? `1px solid ${CC.line}` : 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {rivalCrestSrc(m.rival) ? <RivalCrest rival={m.rival} size={28} /> : <Icon name="versus" size={20} color={CC.navy} sw={2.2} />}
        </div>
